...
class ProjectAgent:
    ...
    def train(self, state, action, reward, next_state, done):
        ...
        if len(self.replay_buffer.buffer) > self.batch_size:
            ...
            next_q_values_local = self.q_network(next_state_batch)
            _, next_actions = next_q_values_local.max(1)
            next_q_values_target = self.target_network(next_state_batch).detach()
...
def train_agent(agent, env, num_episodes=NUM_EPISODES):
    for episode in range(num_episodes):
        ...
        for _ in range(MAX_EPISODE_STEP):
            eps = max(0.1, 0.9 - 0.9 * 2 * episode / NUM_EPISODES)
